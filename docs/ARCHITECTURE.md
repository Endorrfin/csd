...
- **GitHub Actions trigger** (`.github/workflows/deploy.yml`):
    ...
    - The concurrency group is a static **`deploy-prod`** with **`cancel-in-progress: false`**. Merge-triggered and dispatch-triggered runs share one group and therefore execute strictly one at a time, and no prod deploy is ever cancelled mid-flight. A manual dispatch still wins over a *queued* merge run — GitHub cancels the previously pending run in a group — it simply waits for the in-flight one to finish. Aborting mid-flight was rejected deliberately: `migration:run` commits each migration separately (`migrationsTransactionMode: 'each'`, `backend/src/database/data-source.ts`), so an interrupted run strands the schema on a partially applied set; a killed `serverless deploy` leaves CloudFormation in `UPDATE_IN_PROGRESS`, which blocks the next deploy; and a killed `s3 sync --delete` serves 404s for hashed chunks. To stop one specific run, cancel it from the Actions tab. (Until this change the group interpolated `github.event_name`, which put the two event types in **different** groups — the failure mode this replaces.)
