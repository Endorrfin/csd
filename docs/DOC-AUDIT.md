...
>
> All four passes are done. **Every tracked document in this repository has been re-verified against the code**, most recently at `e5a4578` (2026-07-31). This file is now a *record* of that work, not a to-do list — §2's per-document drift tables describe states that no longer exist and are kept only so the reasoning is auditable. **§2's inventory is closed at pass D.** A document that entered the repository later has no drift table here and will not get one — there was no audit of it to record. What such a document owns is written in §5 instead; `docs/CONCERNS.md` is the first case (item 21).
>
> **Read §5 first.** It holds the settled decisions — which document owns what, and what was deliberately not done. Items 1–20 came out of the four passes; items 21+ were settled afterwards, while the staging environment was built, and are recorded in the same place for the same reason. Re-litigating them is how the split falls apart.
>
> **What would make this stale again**, in rough order of likelihood: a new backend module or upload endpoint; a change to any of the **four** GitHub workflows — `.github/workflows/test.yml`, `deploy.yml`, `deploy-staging.yml` and `staging-rds-schedule.yml` (the last two are described in `ARCHITECTURE.md` §12.4 and §12.5, and §12's own intro table is the list to keep in step; the `ui` job this sentence once warned about has since been added to `test.yml`, which is why §5 item 10 already carries a *Superseded* note); applying or enforcing the prepared CloudFront CSP policy; shipping PR-D4 of the About registry; and any change to an npm script, which must be mirrored in `CONTRIBUTING.md` §4. Live-AWS values (§5 item 15) go stale on their own schedule and carry re-check commands.
...
**Decisions settled after the refresh** — taken while the staging environment was built (`CONCERNS.md` P1-5, closed 2026-09-09), not during passes A–D. They are recorded here, and not in §2, because each one answers *which document owns what* — which is what this section is for — rather than describing drift that an audit pass found.

21. **`docs/CONCERNS.md` is tracked, and it owns the prioritised debt ledger.** It entered the repository with PR #163 (`6c1ddaf`, 2026-09-02), a month after pass D, so it has no §2 drift table — see the banner. Its division of labour with `ARCHITECTURE.md` §17 is stated in its own header and is worth not re-deriving: **§17 describes what the debt *is*; `CONCERNS.md` says what to do about it and in what order.** Its item numbers are **stable identifiers** — a closed item is deleted and the rest are *not* renumbered, so a gap in the numbering means something was closed, not that something is missing. Tracked documents may cite it by item number; before it was un-ignored, they could not.
22. **`docs/CONCERNS.md` is written in Ukrainian and every other tracked document is in English.** Deliberate, not drift: it is the one document addressed to the author rather than to a reader of the repository. Do not translate it, and do not lift its phrasing into an English document — cite the item number instead.
23. **The staging hostname never enters a tracked file.** The repository is public and staging has no authentication, so the CloudFront domain and `STAGING_SMOKE_URL` live only in the GitHub `staging` environment. Two consequences that look like bugs and are not: the deploy job summary renders the smoke URL as `https://***/`, because GitHub redacts every occurrence of the `PUBLIC_HOST` secret's value, and `PUBLIC_HOST` therefore **cannot** be moved to a variable to make the summary readable. The distribution id `E2OQ1H0LD6DAVP` and the SSR origin `fyq2zwzcqi.execute-api.eu-central-1.amazonaws.com` are the opposite case — both are committed in `infra/cloudfront-distribution-staging.json` and in git history, so removing them from the tree would undo nothing. Full reasoning: `CONCERNS.md` P1-5.
24. **Exactly two files under `docs/tasks/` are tracked** — `tasks.md` (`09aa0fb`, 2026-06-17) and `doc-refresh-task.md` (`98608f5`, 2026-07-29). Both predate the `/docs/tasks/` line in `.gitignore` (`c254041`, 2026-08-10), which does not apply to already-tracked paths. Everything else in that directory — `PROMPT_*.md`, `ANALYSIS_*.md` — is ignored, and an ignored file is **a separate physical file in each working copy** with nothing reconciling the copies. A tracked document may cite those two files (`CONTRIBUTING.md`'s unnumbered *GitHub Issues (getting started)* section cites `tasks.md`) and nothing else under that directory.
25. **Staging documentation is split by kind and is not gathered into one file.** `ARCHITECTURE.md` §8.2 owns the topology, §8.3 the SSR static-asset invariant, §9 the SSR Lambda's environment variables, §12.4 the `deploy-staging.yml` walkthrough and §12.5 the RDS stop/start schedule; `infra/SECURITY-HEADERS.md` §3 owns the header set staging serves and why it carries no CSP; `CONCERNS.md` P1-5 owns the **status** — what was built, in what order, and what is still open. Same rule as items 9 and 13: link to the owner, do not restate. There is deliberately no `STAGING.md`; a fifth document would duplicate all four and go stale first.
