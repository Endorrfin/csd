...
@Component({
  ...
  template: `
    <div class="admin-layout">
      ...
      <aside class="admin-sidebar" [class.admin-sidebar--open]="isSidebarOpen()">
        ...
        <nav class="admin-nav">
          <a routerLink="wash-forms" routerLinkActive="active" class="admin-nav-item" (click)="closeSidebar()">
            {{ isUa ? 'WASH Заявки' : 'WASH Forms' }}
          </a>
          <a routerLink="procurements" routerLinkActive="active" class="admin-nav-item" (click)="closeSidebar()">
            {{ isUa ? 'Закупки' : 'Procurements' }}
          </a>
          <a routerLink="vacancies" routerLinkActive="active" class="admin-nav-item" (click)="closeSidebar()">
            {{ isUa ? 'Вакансії' : 'Vacancies' }}
          </a>
          <a routerLink="testimonials" routerLinkActive="active" class="admin-nav-item" (click)="closeSidebar()">
            {{ isUa ? 'Відгуки' : 'Testimonials' }}
          </a>

          <!-- Complaints: admin+ (handles sensitive reports) -->
          @if (auth.isAdmin) {
            <a routerLink="complaints" routerLinkActive="active" class="admin-nav-item" (click)="closeSidebar()">
        ...
        @if (auth.isSuperAdmin) {
          ...
          <nav class="admin-nav">
            <a routerLink="users" routerLinkActive="active" class="admin-nav-item" (click)="closeSidebar()">
  ...
  styles: [`
    ...
    .admin-burger--open span:nth-child(1) { transform: translateY(8px) rotate(45deg); }
    .admin-burger--open span:nth-child(2) { opacity: 0; }
    .admin-burger--open span:nth-child(3) { transform: translateY(-8px) rotate(-45deg); }
    ...
    .admin-user { display: flex; flex-direction: column; gap: .35rem; }
    .admin-user-email {
      font-size: .75rem;
      color: #64748b;
    ...
    .admin-nav-item:hover { background: #edf2f7; color: #1a365d; }
    ...
    /* ── Main content ── */
    .admin-content { padding: 2rem 1.5rem; min-width: 0; }
