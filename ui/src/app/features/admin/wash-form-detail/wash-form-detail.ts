        ...
        <div class="section-card">
          ...
          <div class="info-grid">
            ...
            <div class="info-item">
              ...
              ><span>{{ form()!.headName }}</span>
            </div>
            <div class="info-item">
              <span class="info-label">{{ isUa ? 'Телефон' : 'Phone' }}</span
              ><span>{{ form()!.headPhone }}</span>
            </div>
            <div class="info-item">
              <span class="info-label">Email</span><span>{{ form()!.email }}</span>
            </div>
          </div>
        </div>
          ...
          <div class="section-card">
            ...
            <div class="info-grid">
              <div class="info-item">
                ...
                ><span>{{ form()!.boreholeDrilling.workType }}</span>
              </div>
              @if (form()!.boreholeDrilling.expectedFlowRate) {
                <div class="info-item">
                  <span class="info-label">{{ isUa ? 'Очікуваний дебіт' : 'Expected flow' }}</span
                  ><span>{{ form()!.boreholeDrilling.expectedFlowRate }} m³/h</span>
                </div>
              }
              @if (form()!.boreholeDrilling.existingDepth) {
                <div class="info-item">
                  <span class="info-label">{{ isUa ? 'Глибина існуючої' : 'Existing depth' }}</span
                  ><span>{{ form()!.boreholeDrilling.existingDepth }} m</span>
                </div>
              }
              @if (form()!.boreholeDrilling.existingDebit) {
                <div class="info-item">
                  <span class="info-label">{{ isUa ? 'Дебіт існуючої' : 'Existing debit' }}</span
                  ><span>{{ form()!.boreholeDrilling.existingDebit }} m³/h</span>
                </div>
              }
              <div class="info-item">
                <span class="info-label">{{ isUa ? 'Водоносний горизонт' : 'Aquifer info' }}</span
                ><span>{{ form()!.boreholeDrilling.hasAquiferInfo ? 'Так/Yes' : 'Ні/No' }}</span>
              </div>
              <div class="info-item">
                <span class="info-label">{{ isUa ? 'Паспорт' : 'Passport' }}</span
                ><span>{{ form()!.boreholeDrilling.hasPassport ? 'Так/Yes' : 'Ні/No' }}</span>
              </div>
              <div class="info-item">
                <span class="info-label">{{ isUa ? 'Інфо про конструкцію' : 'Design info' }}</span
                ><span>{{ form()!.boreholeDrilling.hasDesignInfo ? 'Так/Yes' : 'Ні/No' }}</span>
              </div>
              @if (form()!.boreholeDrilling.oldLocation) {
                <div class="info-item full-w">
                  <span class="info-label">{{ isUa ? 'Розташування старої' : 'Old location' }}</span
                  ><span>{{ form()!.boreholeDrilling.oldLocation }}</span>
                </div>
              }
              @if (form()!.boreholeDrilling.notes) {
                <div class="info-item full-w">
                  <span class="info-label">{{ isUa ? 'Примітки' : 'Notes' }}</span
                  ><span>{{ form()!.boreholeDrilling.notes }}</span>
                </div>
              }
            </div>
          </div>
          ...
          <div class="section-card">
            ...
            <div class="info-grid">
              <div class="info-item">
                ...
                ><span>{{ form()!.waterTower.towerType }}</span>
              </div>
              <div class="info-item">
                <span class="info-label">{{ isUa ? 'Висота' : 'Height' }}</span
                ><span>{{
                  form()!.waterTower.towerHeight === 'over_25'
                    ? (form()!.waterTower.customHeight || 'Over 25') + ' m'
                    : form()!.waterTower.towerHeight + ' m'
                }}</span>
              </div>
              <div class="info-item">
                <span class="info-label">{{ isUa ? 'Фундамент' : 'Foundation' }}</span
                ><span>{{ form()!.waterTower.hasFoundation ? 'Так/Yes' : 'Ні/No' }}</span>
              </div>
              <div class="info-item">
                <span class="info-label">{{ isUa ? 'Фундамент придатний' : 'Suitable' }}</span
                ><span>{{ form()!.waterTower.isFoundationSuitable ? 'Так/Yes' : 'Ні/No' }}</span>
              </div>
              <div class="info-item">
                ...
                ><span>{{
                  form()!.waterTower.needsFoundationReconstruction ? 'Так/Yes' : 'Ні/No'
                }}</span>
              </div>
              <div class="info-item">
                <span class="info-label">{{ isUa ? 'Власними силами' : 'Self-reconstruct' }}</span
                ><span>{{ form()!.waterTower.canSelfReconstruct ? 'Так/Yes' : 'Ні/No' }}</span>
              </div>
              <div class="info-item">
                <span class="info-label">{{ isUa ? 'Кран' : 'Crane' }}</span
                ><span>{{ form()!.waterTower.canProvideCrane ? 'Так/Yes' : 'Ні/No' }}</span>
              </div>
              @if (form()!.waterTower.notes) {
                <div class="info-item full-w">
                  <span class="info-label">{{ isUa ? 'Примітки' : 'Notes' }}</span
                  ><span>{{ form()!.waterTower.notes }}</span>
                </div>
              }
            </div>
          </div>
          ...
          <div class="section-card">
            ...
            <div class="info-grid">
              <div class="info-item">
                ...
                ><span>{{ form()!.purificationSystem.hasRoom ? 'Так/Yes' : 'Ні/No' }}</span>
              </div>
              <div class="info-item">
                <span class="info-label">{{ isUa ? 'Температура' : 'Temp' }}</span
                ><span>{{
                  form()!.purificationSystem.hasTemperatureControl ? 'Так/Yes' : 'Ні/No'
                }}</span>
              </div>
              <div class="info-item">
                <span class="info-label">{{ isUa ? 'Водопостачання' : 'Water' }}</span
                ><span>{{
                  form()!.purificationSystem.hasWaterInletDrainage ? 'Так/Yes' : 'Ні/No'
                }}</span>
              </div>
              <div class="info-item">
                <span class="info-label">{{ isUa ? 'Електрика' : 'Power' }}</span
                ><span>{{ form()!.purificationSystem.hasPowerSupply ? 'Так/Yes' : 'Ні/No' }}</span>
              </div>
              <div class="info-item">
                <span class="info-label">{{ isUa ? 'Обслуговування' : 'Maintenance' }}</span
                ><span>{{
                  form()!.purificationSystem.canMaintainSystem ? 'Так/Yes' : 'Ні/No'
                }}</span>
              </div>
              <div class="info-item">
                <span class="info-label">{{ isUa ? 'Надання води жителям' : 'Provide water' }}</span
                ><span>{{
                  form()!.purificationSystem.willingToProvideWater ? 'Так/Yes' : 'Ні/No'
                }}</span>
              </div>
              @if (form()!.purificationSystem.notes) {
                <div class="info-item full-w">
                  <span class="info-label">{{ isUa ? 'Примітки' : 'Notes' }}</span
                  ><span>{{ form()!.purificationSystem.notes }}</span>
                </div>
              }
            </div>
          </div>
                  ...
                  @for (item of form()!.items; track item.id; let i = $index) {
                    <tr>
                      ...
                      <td>
                        {{
                          isUa
                            ? item.equipmentItem?.category?.nameUa
                            : item.equipmentItem?.category?.nameEn
                        }}
                      </td>
