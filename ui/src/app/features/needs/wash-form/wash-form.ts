        ...
        @if (currentStep() === 0) {
          <div class="form-section">
            ...
            <div class="form-grid">
              ...
              <div class="form-field">
                <label
                  >{{ isUa ? 'Телефон керівника' : 'Head phone' }} <span class="req">*</span></label
                >
                <input formControlName="headPhone" type="tel" placeholder="+380XXXXXXXXX" />
                @if (showError('headPhone')) {
                  <span class="error">{{ isUa ? 'Мін. 6 символів' : 'Min 6 chars' }}</span>
                }
              </div>
              <div class="form-field">
            ...
            <div class="section-header-optional">
              <div>
                ...
                <p class="section-hint">
                  {{
                    isUa
                      ? 'Заповніть, якщо потрібне буріння. Інакше натисніть Пропустити.'
                      : 'Fill if drilling is needed. Otherwise click Skip.'
                  }}
                </p>
              </div>
            ...
            <div class="form-grid">
              ...
              @if (form.value.boreholeDrilling?.workType === 'new_near_existing') {
                <div class="form-field">
                  <label>{{
                    isUa
                      ? 'Чи є відомості про водоносний горизонт?'
                      : 'Do you have aquifer information?'
                  }}</label>
                ...
                <div class="form-field">
                  <label>{{
                    isUa ? 'Глибина існуючої свердловини, м' : 'Existing borehole depth, m'
                  }}</label>
                  <input
                    type="number"
                    formControlName="existingDepth"
                    min="1"
                    max="280"
                    placeholder="1 - 280"
                  />
                ...
                <div class="form-field">
                  <label>{{
                    isUa ? 'Дебіт існуючої свердловини, м³/год' : 'Existing borehole debit, m³/h'
                  }}</label>
                  <input
                    type="number"
                    formControlName="existingDebit"
                    min="1"
                    max="30"
                    placeholder="1 - 30"
                  />
                ...
                <div class="form-field">
                  <label>{{
                    isUa
                      ? 'Чи є інформація про конструкцію свердловини, її обладнанання?'
                ...
                <div class="form-field">
                  <label>{{
                    isUa
                      ? 'Чи є паспорт на існуючу свердловину?'
                      : 'Passport for existing borehole?'
                  }}</label>
                ...
                <div class="form-field full-width">
                  <label>{{
                    isUa
                      ? 'Де розташована стара свердловина?'
                      : 'Where is the old borehole located?'
                  }}</label>
                  <input
                    formControlName="oldLocation"
                    [placeholder]="isUa ? 'Опишіть розташування' : 'Describe location'"
                  />
                </div>
              }
              ...
              <div class="form-field">
                <label>{{
                  isUa
                    ? 'Очікуваний дебіт після виконання робіт, м³/год'
                    : 'Expected flow rate after work, m³/h'
                }}</label>
                <input
                  type="number"
                  formControlName="expectedFlowRate"
                  min="1"
                  max="50"
                  placeholder="1 - 50"
                />
              ...
              <div class="form-field full-width">
                ...
                <textarea
                  ...
                  rows="2"
                  [placeholder]="isUa ? 'Додаткова інформація' : 'Additional information'"
                ></textarea>
              </div>
            </div>
          </div>
            ...
            <div class="section-header-optional">
              <div>
                ...
                <p class="section-hint">
                  {{
                    isUa
                      ? 'ВБР класифікуються за обʼємом бака та висотою опори.'
                      : 'VBRs classified by tank volume and support height.'
                  }}
                </p>
              </div>
              <span class="badge-optional">{{ isUa ? 'опціонально' : 'optional' }}</span>
            ...
            <div class="form-grid">
              <div class="form-field">
                ...
                <select formControlName="towerType">
                  ...
                  <option value="vbr_over_50">
                    {{ isUa ? 'ВБР понад 50 м³' : 'VBR over 50 m³' }}
                  </option>
                </select>
              </div>
              <div class="form-field">
            ...
            <div class="checklist" style="margin-top:1rem">
              <label class="checklist-item">
                ...
                <span class="checklist-text">{{
                  isUa
                    ? 'Чи є фундамент під Водонапірну башту?'
                    : 'Is there a foundation for the Water Tower?'
                }}</span>
              </label>
              <label class="checklist-item">
                ...
                <span class="checklist-text">{{
                  isUa
                    ? 'Чи придатний фундамент для установки вибраної башти? (візуальна оцінка)'
                    : 'Is the foundation suitable for installation? (visual assessment)'
                }}</span>
              </label>
              <label class="checklist-item">
                ...
                <span class="checklist-text">{{
                  isUa
                    ? 'Чи є потреба в реконструкції або будівництві нового фундаменту?'
                    : 'Need for reconstruction or new foundation construction?'
                }}</span>
              </label>
              <label class="checklist-item">
                ...
                <span class="checklist-text">{{
                  isUa
                    ? 'Чи є можливість виконати реконструкцію / будівництво фундаменту власними силами?'
                    : 'Can you carry out foundation reconstruction on your own?'
                }}</span>
              </label>
              <label class="checklist-item">
                ...
                <span class="checklist-text">{{
                  isUa
                    ? 'Чи є можливість надати кран для демонтажу / установки башти?'
                    : 'Can you provide a crane for dismantling/installing the tower?'
                }}</span>
              </label>
            </div>
          ...
          <div class="form-section" formGroupName="purificationSystem">
            <div class="section-header-optional">
              <div>
                ...
                <p class="section-hint">
                  {{
                    isUa
                      ? 'Вкажіть наявність необхідних умов для встановлення.'
                      : 'Indicate availability of required conditions.'
                  }}
                </p>
              </div>
              <span class="badge-optional">{{ isUa ? 'опціонально' : 'optional' }}</span>
            ...
            <div class="checklist">
              <label class="checklist-item">
                ...
                <span class="checklist-text">{{
                  isUa
                    ? 'Наявність приміщення (площа 8-10 м2, висота не менше 2.7 м)'
                    : 'Room available (area 8-10 m2, height at least 2.7 m)'
                }}</span>
              </label>
              <label class="checklist-item">
                ...
                <span class="checklist-text">{{
                  isUa
                    ? 'Температура в приміщенні від +5C до +30C'
                    : 'Room temperature from +5C to +30C'
                }}</span>
              </label>
              <label class="checklist-item">
                ...
                <span class="checklist-text">{{
                  isUa
                    ? 'Наявність вхідного водопостачання та дренажу для зворотного промивання'
                    : 'Water inlet and drainage for backwashing available'
                }}</span>
              </label>
              <label class="checklist-item">
                ...
                <span class="checklist-text">{{
                  isUa
                    ? 'Електроживлення 220В та достатня кількість розеток'
                    : '220V power supply and sufficient sockets'
                }}</span>
              </label>
              <label class="checklist-item">
                ...
                <span class="checklist-text">{{
                  isUa
                    ? 'Спроможність обслуговування системи (оплата електроенергії, щорічне обслуговування, заміна витратних матеріалів)'
                    : 'Ability to maintain system (electricity, annual maintenance, consumables replacement)'
                }}</span>
              </label>
              <label class="checklist-item">
                ...
                <span class="checklist-text">{{
                  isUa
                    ? 'Готовність надавати очищену воду місцевим жителям, а не лише використовувати для власних потреб'
                    : 'Willingness to provide purified water to local residents, not only for own needs'
                }}</span>
              </label>
            </div>
            <div class="form-grid" style="margin-top: 1rem">
        ...
        @if (currentStep() === 5) {
          <div class="form-section">
            <div class="section-header-optional">
              <div>
                <h2 class="section-title">
                  {{ isUa ? 'VI. Обладнання та матеріали' : 'VI. Equipment & Materials' }}
                </h2>
                <p class="section-hint">
                  {{
                    isUa
                      ? 'Оберіть категорію, позицію з каталогу та вкажіть кількість.'
                      : 'Select category, item and specify quantity.'
                  }}
                </p>
              </div>
              <span class="badge-optional">{{ isUa ? 'опціонально' : 'optional' }}</span>
                  ...
                  <div class="equipment-row" [formGroupName]="idx">
                    ...
                    <div class="equipment-row-fields">
                      ...
                      <div class="form-field">
                        ...
                        @if (isDuplicate(idx)) {
                          <span class="warning-text">{{
                            isUa ? 'Ця позиція вже додана вище' : 'This item is already added above'
                          }}</span>
                        }
                      </div>
                      <div class="form-field form-field-sm">
                        <label
                          >{{ isUa ? 'К-сть' : 'Qty' }}
                          @if (getSelectedItemUnit(idx); as u) {
                            <span class="unit-hint">({{ u }})</span>
                          }
                        </label>
        ...
        @if (currentStep() === 6) {
          <div class="form-section">
              ...
              <div class="review-block">
                ...
                <div class="review-grid">
                  <div class="review-item">
                    ...
                    ><span class="review-value">{{ getBoreholeTypeLabel() }}</span>
                  </div>
                  @if (form.value.boreholeDrilling?.expectedFlowRate) {
                    <div class="review-item">
                      ...
                      ><span class="review-value"
                        >{{ form.value.boreholeDrilling?.expectedFlowRate }} m³/h</span
                      >
                    </div>
                  }
                  @if (form.value.boreholeDrilling?.workType === 'new_near_existing') {
                    @if (form.value.boreholeDrilling?.existingDepth) {
                      <div class="review-item">
                        ...
                        ><span class="review-value"
                          >{{ form.value.boreholeDrilling?.existingDepth }} m</span
                        >
                      </div>
                    }
                    @if (form.value.boreholeDrilling?.existingDebit) {
                      <div class="review-item">
                        ...
                        ><span class="review-value"
                          >{{ form.value.boreholeDrilling?.existingDebit }} m³/h</span
                        >
                      </div>
                    }
                    <div class="review-item">
                      <span class="review-label">{{
                        isUa ? 'Водоносний горизонт' : 'Aquifer info'
                      }}</span
                      ><span class="review-value">{{
                        form.value.boreholeDrilling?.hasAquiferInfo
                          ? isUa
                            ? 'Так'
                            : 'Yes'
                          : isUa
                    ...
                    <div class="review-item">
                      <span class="review-label">{{ isUa ? 'Паспорт' : 'Passport' }}</span
                      ><span class="review-value">{{
                        form.value.boreholeDrilling?.hasPassport
                          ? isUa
                            ? 'Так'
                            : 'Yes'
                          : isUa
              ...
              <div class="review-block">
                ...
                <div class="review-grid">
                  <div class="review-item">
                    ...
                    ><span class="review-value">{{ getWaterTowerTypeLabel() }}</span>
                  </div>
                  <div class="review-item">
                    <span class="review-label">{{ isUa ? 'Висота' : 'Height' }}</span
                    ><span class="review-value">{{ getWaterTowerHeightLabel() }}</span>
                  </div>
                  <div class="review-item">
                    <span class="review-label">{{ isUa ? 'Фундамент' : 'Foundation' }}</span
                    ><span class="review-value">{{
                      form.value.waterTower?.hasFoundation
                        ? isUa
                          ? 'Так'
                          : 'Yes'
                        : isUa
                  ...
                  <div class="review-item">
                    <span class="review-label">{{
                      isUa ? 'Фундамент придатний' : 'Foundation suitable'
                    }}</span
                    ><span class="review-value">{{
                      form.value.waterTower?.isFoundationSuitable
                        ? isUa
                          ? 'Так'
                          : 'Yes'
                        : isUa
                  ...
                  <div class="review-item">
                    <span class="review-label">{{
                      isUa ? 'Потреба реконструкції' : 'Needs reconstruction'
                    }}</span
                    ><span class="review-value">{{
                      form.value.waterTower?.needsFoundationReconstruction
                        ? isUa
                          ? 'Так'
                          : 'Yes'
                        : isUa
                  ...
                  <div class="review-item">
                    <span class="review-label">{{ isUa ? 'Кран' : 'Crane' }}</span
                    ><span class="review-value">{{
                      form.value.waterTower?.canProvideCrane
                        ? isUa
                          ? 'Так'
                          : 'Yes'
                        : isUa
              ...
              <div class="review-block">
                ...
                <div class="review-grid">
                  <div class="review-item">
                    ...
                    ><span class="review-value">{{
                      form.value.purificationSystem?.hasRoom
                        ? isUa
                          ? 'Так'
                          : 'Yes'
                        : isUa
                  ...
                  <div class="review-item">
                    <span class="review-label">{{ isUa ? 'Температура' : 'Temp' }}</span
                    ><span class="review-value">{{
                      form.value.purificationSystem?.hasTemperatureControl
                        ? isUa
                          ? 'Так'
                          : 'Yes'
                        : isUa
                  ...
                  <div class="review-item">
                    <span class="review-label">{{ isUa ? 'Водопостачання' : 'Water' }}</span
                    ><span class="review-value">{{
                      form.value.purificationSystem?.hasWaterInletDrainage
                        ? isUa
                          ? 'Так'
                          : 'Yes'
                        : isUa
                  ...
                  <div class="review-item">
                    ...
                    ><span class="review-value">{{
                      form.value.purificationSystem?.hasPowerSupply
                        ? isUa
                          ? 'Так'
                          : 'Yes'
                        : isUa
                  ...
                  <div class="review-item">
                    <span class="review-label">{{ isUa ? 'Обслуговування' : 'Maintenance' }}</span
                    ><span class="review-value">{{
                      form.value.purificationSystem?.canMaintainSystem
                        ? isUa
                          ? 'Так'
                          : 'Yes'
                        : isUa
                  ...
                  <div class="review-item">
                    <span class="review-label">{{
                      isUa ? 'Надання води жителям' : 'Provide water'
                    }}</span
                    ><span class="review-value">{{
                      form.value.purificationSystem?.willingToProvideWater
                        ? isUa
                          ? 'Так'
                          : 'Yes'
                        : isUa
            ...
            @if (hasEquipmentItems()) {
              <div class="review-block">
                <h3>
                  {{ isUa ? 'VI. Обладнання' : 'VI. Equipment' }} ({{ getFilledItemsCount() }}
                  {{ isUa ? 'поз.' : 'items' }})
                </h3>
...
export class WashFormComponent implements OnInit {
  ...
  form: FormGroup = this.fb.group({
    ...
    boreholeDrilling: this.fb.group({
      workType: [''],
      ...
      hasPassport: [false],
      oldLocation: [''],
      expectedFlowRate: [null],
      notes: [''],
    }),
    waterTower: this.fb.group({
      towerType: [''],
      towerHeight: [''],
      customHeight: [null],
      hasFoundation: [false],
    ...
    purificationSystem: this.fb.group({
      hasRoom: [false],
  ...
  getSearchText(i: number): string {
    return this.searchTexts.get(i) ?? '';
  }
  ...
  getFilteredItems(i: number): EquipmentItem[] {
    ...
    if (search) {
      items = items.filter(
        (item) =>
          item.nameUa.toLowerCase().includes(search) || item.nameEn.toLowerCase().includes(search),
      );
    }
    return items;
  }
  ...
  getItemNameById(id: string): string {
    ...
    return i ? (this.isUa ? i.nameUa : i.nameEn) : '---';
  }
  getItemUnitById(id: string): string {
    const i = this.itemsMap.get(id);
    return i ? getUnitLabel(i.unit, this.isUa) : '';
  }

  isBoreholeFilled(): boolean {
    const v = this.form.value.boreholeDrilling;
    return !!v?.workType;
  }
  isWaterTowerFilled(): boolean {
    const v = this.form.value.waterTower;
    return !!v?.towerType;
  }
  isPurificationFilled(): boolean {
    const v = this.form.value.purificationSystem;
    return !!(
      v?.hasRoom ||
      v?.hasTemperatureControl ||
      v?.hasWaterInletDrainage ||
      v?.hasPowerSupply ||
      v?.canMaintainSystem ||
      v?.willingToProvideWater
    );
  }
  hasEquipmentItems(): boolean {
    return this.getFilledItemsCount() > 0;
  }
  ...
  getFilledItemsCount(): number {
    return this.itemsArray.controls.filter((c) => c.value.equipmentItemId && c.value.quantity)
      .length;
  }
  getFilledItems() {
    return this.itemsArray.controls.filter((c) => c.value.equipmentItemId && c.value.quantity);
  ...
  getWaterTowerHeightLabel(): string {
    ...
    if (!h) return '---';
    if (h === 'over_25')
      return this.form.value.waterTower?.customHeight
        ? `${this.form.value.waterTower.customHeight} m`
        : this.isUa
          ? 'Понад 25 м'
          : 'Over 25 m';
    return `${h} m`;
  ...
  onSubmit(): void {
    ...
    const filled = v.items?.filter(
      (i: { equipmentItemId: string; quantity: number }) => i.equipmentItemId && i.quantity,
    );
    if (filled?.length) {
      payload.items = filled.map(
        (i: { equipmentItemId: string; quantity: number; notes: string }) => ({
