  ...
  async exportCsv(options?: {
    ...
    const headers = ua
      ? [
          ...
          'Email',
          'Назва обʼєкту',
          'Кількість залежних людей',
          'Соціальні установи',
          'Термін монтажу',
          'Причини заміни',
          'Варіант буріння',
          'Очікуваний дебіт (м³/год)',
          'Глибина існуючої (м)',
          'Дебіт існуючої (м³/год)',
          'Паспорт свердловини',
          'Тип башти',
          'Висота башти',
          'Фундамент',
          ...
          'Кран',
          'Очищення: Приміщення',
          'Очищення: Температура',
          'Очищення: Водопостачання/Дренаж',
          'Очищення: Електрика',
          'Очищення: Обслуговування',
          'Очищення: Надання води жителям',
          'К-сть позицій обладнання',
          'Деталі обладнання',
          'Нотатки менеджера',
        ]
      : [
          ...
          'Purification: Water/Drainage',
          'Purification: Power',
          'Purification: Maintenance',
          'Purification: Provide Water',
    ...
    const rows = forms.map((f) => {
      ...
      const equipDetails = f.items
        ?.map(
          (i) =>
            `${i.equipmentItem?.[nameField] ?? (ua ? 'Невідомо' : 'Unknown')}: ${i.quantity} ${i.equipmentItem?.unit ?? ''}`,
        )
        .join('; ');

      const heightLabel =
        wt?.towerHeight === 'over_25'
          ? wt?.customHeight
            ? `${wt.customHeight} m`
            : ua
              ? 'Понад 25 м'
      ...
      return [
        ...
        f.replacementReason,
        lbl(workTypeLabels, bh?.workType),
        bh?.expectedFlowRate ?? '',
        bh?.existingDepth ?? '',
        bh?.existingDebit ?? '',
        bh ? bool(bh.hasPassport) : '',
        lbl(towerLabels, wt?.towerType),
        heightLabel,
        wt ? bool(wt.hasFoundation) : '',
        wt ? bool(wt.isFoundationSuitable) : '',
        wt ? bool(wt.needsFoundationReconstruction) : '',
        wt ? bool(wt.canProvideCrane) : '',
        ps ? bool(ps.hasRoom) : '',
        ps ? bool(ps.hasTemperatureControl) : '',
        ps ? bool(ps.hasWaterInletDrainage) : '',
        ps ? bool(ps.hasPowerSupply) : '',
        ps ? bool(ps.canMaintainSystem) : '',
        ps ? bool(ps.willingToProvideWater) : '',
        f.items?.length ?? 0,
        equipDetails ?? '',
        f.managerNotes ?? '',
      ]
